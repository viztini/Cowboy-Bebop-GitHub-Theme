...
*   **Cowboy Bebop Styled Color Palette:** Dark, moody blue/black backgrounds, bold jazzy yellow accents, and retro muted red tones.
...
*   **Custom Background Support:** Easily integrate your favorite Bebop-themed background image.
...
4.  **Load Temporary Add-on:**
    ...
    *   Navigate to the directory where you have the Cowboy-Bebop-GitHub-Theme folder.
    *   Select any file *inside* the folder (e.g., `manifest.json` or `content.js`). Firefox will automatically detect the extension.
5.  **Verify Installation:**
    *   The "Cowboy-Bebop-GitHub-Theme" should now appear in your list of temporary add-ons.
6.  **Test the Theme:**
    *   Open a new tab and navigate to `github.com`.
    *   You should see the Bebop-themed colors and styling applied to the GitHub interface.
...
2.  **Save the Image:** Download the image and save it as `bebop-background.png` inside an `icons` directory.
3.  **Reload Extension:** Go to `about:debugging#/runtime/this-firefox` in Firefox, find "Cowboy-Bebop-GitHub-Theme", and click "Reload".
4.  **Refresh GitHub:** Refresh any open GitHub pages to see your new background.

## Contributing

Feel free to modify the `bebop.css` file to further customize the theme to your liking. Experiment with colors, fonts, and element styles!
